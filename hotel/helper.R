process.data <- function(current_repre,current_thres){
  ...
  #Read raw data from all files
  filenames <- list.files(paste0(getwd(),"/Data"), pattern="*.dat", full.names=TRUE)
  data2 <- NULL
  ...
  for (file in filenames){
    ...
    data$Original <- as.character(data$Original)
    data$Header <- unlist(lapply(data$Original, function(x) ifelse(substr(x,1,1)=="<",
                                                                   strsplit(x,"[<>]")[[1]][[2]], "None")))
    #Remove rows not starting with <
    if(length(which(substr(data$Original,1,1)!="<")) != 0){
      data <- data[-which(substr(data$Original,1,1)!="<"),]
    }
    data$Dummy <- unlist(lapply(data$Original, function(x)
      ifelse(substr(x,1,4) %in% c("<Ove","<Con","<Dat"), 1, 0)))
    data <- data[data$Dummy == 1,]
    data$Dummy <- NULL
    data$Content <- unlist(lapply(data$Original, function(x) ifelse(substr(x,1,1)=="<",
                                                                    strsplit(x,">")[[1]][[2]], "None")))
    data$Original <- NULL
    #Build dataframe with review and overall rating
    Review <- NULL
    Rating <- NULL
    Date <- NULL
    for(i in 1:nrow(data)){
      if(data[i,"Header"] == "Content"){
        Review <- c(Review, data[i,"Content"])
      }
      if(data[i,"Header"] == "Overall"){
        Rating <- c(Rating, data[i,"Content"])
      }
      if(data[i,"Header"] == "Date"){
        Date <- c(Date, data[i,"Content"])
      }
    }
    #Check if equal length
    if(length(Review) < length(Rating)){
      Rating <- Rating[1:length(Review)]
    }
    if(length(Review) > length(Rating)){
      Review <- Review[1:length(Rating)]
    }
    tempData <- data.frame(Review, Rating, Date)
  ...
  data2$Review <- as.character(data2$Review)
  data2$Rating <- as.numeric(as.character(data2$Rating))
  #Prepare dataset for modelling
  data <- data2
  cat("\nScore frequency:\n")
  cat(table(data$Rating))
  data$Sentiment <- ifelse(data$Rating > 3, "positive",
                           ifelse(data$Rating < 3, "negative","none"))
  ...
  head(data$ReviewLength)
  #Remove unrecognized reviews (i.e. containing Ã)
  data <- data[-c(137,1042,1606,1607,1611,1613,2075,2081),]
  cat("\nDataframe dimension after revoming outliers:\n")
  cat(dim(data))
  cat("\nSentiment frequency after removing outliers:\n")
  cat(table(data$Sentiment))
  temp.backup <- data
  data$Rating <- NULL
  data <- data[,c("Review","Sentiment","ReviewLength")]
  cat("\nDataframe dimension:\n")
  cat(dim(data))
  cat("\nSentiments:\n")
  cat(table(data$Sentiment))
